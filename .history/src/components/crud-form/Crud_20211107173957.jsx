...
export default function Crud({
  ...
  useEffect(() => {
    async function getTareas(coleccion, array, setter) {
      ...
      await snapshot.forEach( ele =>{
         array.push({ id: ele.id, nameTarea: ele.data().nameTarea });
        setter(array);
      })
    }
  ...
  const agregarTareas = async (e) => {
    ...
    if (!tarea.trim()) {
     ...
     addOrEdit(tarea, idEdited, "tareas");
    setError(null);
    setIdEdited(nanoid());
    setTarea("");
  }; //agregarTareas
  ...
  const completarTarea = async (item) => {
     borrarDatos(item.id, "tareas");
    await setTareascompletadas([
      ...tareasCompletadas,
      {
        id: item.id,
    ...
    await addOrEdit(item.nameTarea, item.id, "completed");
    setTareas(array);
  };
