...
export default function Crud({ correoUsuario }) {
  ...
  async function searchDocOrCreate(id) {
    //crear referencia al documento

    const docRef = doc(firestore, `task/${id}`);
    //buscar documento
    ...
    if (data.exists()) {
      ...
      await setDoc(docRef, { tareas: [...tareas] });
      const data = await getDoc(docRef);
      const infoDoc = data.data();
      return infoDoc.tareas;
    }
  ...
  useEffect(() => {
    async function fetchTareas() {
      const data = await searchDocOrCreate(correoUsuario);
      setTareas(data);
    }
    fetchTareas();
