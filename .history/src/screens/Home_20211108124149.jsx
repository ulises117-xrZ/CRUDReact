...
export default function Home({ correoUsuario }) {
  ...
  const getData = async ()=>{
    const docSnap = await getDoc(doc(db, 'tareas', correoUsuario));
    if(docSnap.exists()){
      console.log("document.data", docSnap.data().tareas);
      // return docSnap.data();
    } else{
  ...
  return (
    <>
      <Crud correoUsuario={correoUsuario} createData={createData} getData = {getData}/>
      <Button
        variant="warning"
        type="submit"
        onClick={() => {
